...
const getEditorComponentById = (editorProgram: EditorProgram, id: string): EditorProgramComponent | undefined => {
...
const getEditorComponentByIdFromSection = (
  ...
  const innerStepsResult = editorSection.innerSteps
    ...
    .reduce((prev, curr) => (prev ? prev : curr), undefined);
  const endStepResult = editorSection.endStep ? getEditorComponentByIdFromStep(editorSection.endStep, id) : undefined;
  return innerStepsResult ? innerStepsResult : endStepResult;
};

const getEditorComponentByIdFromStep = (editorStep: EditorStep, id: string): EditorProgramComponent | undefined => {
  if (editorStep.id == id) {
    return editorStep;
  }
  if (editorStep.type == EditorStepType.UserDecision) {
    const choice1Result = getEditorComponentByIdFromSection(editorStep.choice1, id);
    const choice2Result = getEditorComponentByIdFromSection(editorStep.choice2, id);
