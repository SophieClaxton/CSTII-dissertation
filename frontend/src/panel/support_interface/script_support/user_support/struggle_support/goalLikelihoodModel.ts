...
const defautlStruggleProbEquations: Record<UserSupportGoal, StruggleProbEqn> = {
  ...
  dec: (struggleProb, k) => 1.5 * Math.pow(struggleProb - (20 + k) / 20, 2),
  none: (struggleProb, k) =>
    3 *
    Math.exp(
      -20 * Math.exp(-0.2 * k) * Math.pow(struggleProb - (5 + k) / 20, 2),
    ),
};
...
const computeMovingAverage = (
  ...
  return {
    ...
    deltaStepsCompleted:
      alpha * newEvidence.deltaStepsCompleted +
      (1 - alpha) * lastAverage.deltaStepsCompleted,
  };
};
...
const computeUserGoalProbabilities = (
  ...
  const k = movingEvidenceAverage.deltaStepsCompleted;
